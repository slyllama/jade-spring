...
func reveal() -> void:
	...
	scale_tween.tween_property(self, "scale", Vector2(0.5, 0.5), 0.3)
	var paint_tween = create_tween()
	paint_tween.tween_method(_set_paint_value, 0.0, 1.0, 0.3)
	paint_tween.set_parallel()
	
	await get_tree().create_timer(3.0).timeout
	
	var scale_tween2 = create_tween()
	scale_tween2.tween_property(self, "scale", Vector2(0.7, 0.7), 0.3)
	var paint_tween2 = create_tween()
	paint_tween2.tween_method(_set_paint_value, 0.1, 0.0, 0.3)
