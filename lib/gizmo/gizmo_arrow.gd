...
func _ready() -> void:
	...
	adjacent_plane.set_collision_layer_value(3, 0)
	adjacent_plane.set_size(Vector3(0.4, 0.4, EXTENTS))
	...
	adjacent_plane.rotation.z = deg_to_rad(22.0)
	...
	if active and get_drag_collision() != null:
		if tangent_cast.get_collider() != null:
			if last_collision == null:
				...
				if !Global.snapping:
					get_parent().global_position = lerp(get_parent().global_position, _new_pos, delta * 13.5)
				else:
